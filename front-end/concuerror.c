 ...
 *  Description : Concuerror front-end.
 * ----------------------------------------------------------------------
 */
...
static void free_env_val(char *value)
{
#ifdef __WIN32__
    if (value)
        free(value);
...
int main(int argc, char** argv)
{
    ...
    env = get_env("CONCUERROR_EMULATOR");
    emulator = env ? env : get_default_emulator(argv[0]);

    if (strlen(emulator) >= MAXPATHLEN)
        error("Value of environment variable CONCUERROR_EMULATOR is too large");
...
static int run_erlang(char* progname, char** argv)
{
#ifdef __WIN32__
    int status;
#endif
